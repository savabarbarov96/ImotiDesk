...
import { ArrowLeft, Save, Upload, X, MapPin, Home } from 'lucide-react';
...
import { useCreateProperty, type PropertyFormData } from '@/hooks/use-properties';
...
import { movePropertyImages } from '@/utils/storageHelpers';
...
const AddPropertyPage = () => {
  ...
  const [sellRequestData, setSellRequestData] = useState<any>(null);
  const [temporaryId] = useState(() => `temp_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`);

  // Check for sell request conversion data on component mount
  useEffect(() => {
    const convertData = sessionStorage.getItem('convertSellRequest');
    if (convertData) {
      try {
        const data = JSON.parse(convertData);
        setSellRequestData(data);
        
        // Pre-fill the form with sell request data
        setFormData(prev => ({
          ...prev,
          title: data.title || '',
        ...
        toast({
          title: 'Конвертиране на заявка за продажба',
          description: `Формулярът е попълнен с данни от заявката на ${data.contact_name}. Можете да редактирате информацията преди запазване.`,
        });

        // Clear the session storage
        sessionStorage.removeItem('convertSellRequest');
      } catch (error) {
        console.error('Error parsing sell request data:', error);
      }
    }
  }, [toast]);
  ...
  const handleImageDelete = async (url: string) => {
    try {
      ...
      const filePath = `property_media/${temporaryId}/${fileName}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const createdProperty = await createProperty.mutateAsync(propertyData as PropertyFormData);

      // Move images from temporary folder to final property folder
      if (uploadedImages.length > 0 && createdProperty.id) {
        try {
          const movedImageUrls = await movePropertyImages(temporaryId, createdProperty.id);
          ...
          // Don't fail the whole operation if image moving fails
        }
      }

      // If this property was created from a sell request, mark it as converted
      if (sellRequestData?.id) {
        try {
          const { error: updateError } = await supabase
            .from('sell_requests')
            .update({ status: 'converted' })
            .eq('id', sellRequestData.id);
          ...
          console.error('Error updating sell request:', updateErr);
        }
      }

      toast({
        title: 'Успешно създаване',
        description: sellRequestData 
          ? `Имотът беше създаден успешно и заявката на ${sellRequestData.contact_name} беше маркирана като конвертирана.`
          : 'Имотът беше създаден успешно.',
      });
      ...
      toast({
        ...
        description: 'Възникна грешка при създаването на имота.',
        variant: 'destructive'
      });
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Helmet>
        <title>Добави нов имот | Automation Aid</title>
      </Helmet>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-xl font-semibold text-gray-900">Добави нов имот</h1>
            </div>
            <Button
            ...
            >
              ...
              {isSubmitting ? 'Запазване...' : 'Запази имота'}
            </Button>
          </div>
        </div>
      </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="text-sm font-medium text-blue-800">
                  Конвертиране на заявка за продажба
                </h3>
                <p className="text-sm text-blue-700 mt-1">
                  Създавате имот от заявката на <strong>{sellRequestData.contact_name}</strong>.
                  Формулярът е попълнен с данните от заявката. Можете да редактирате информацията преди запазване.
                </p>
                <div className="mt-2 text-xs text-blue-600">
                  <p>Контакт: {sellRequestData.contact_email} | {sellRequestData.contact_phone}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {isSofia && (
                      <div>
                        <Label htmlFor="sub_district">Район (София)</Label>
                        <Select
                          value={formData.sub_district || ''}
                          onValueChange={(value) => handleInputChange('sub_district', value)}
                        >
                          <SelectTrigger>
                            <SelectValue placeholder="Изберете район" />
                          </SelectTrigger>
                          <SelectContent>
              ...
              <Card>
                ...
                <CardContent>
                  <ImageUploader
                    bucketName="trendimo"
                    folderPath={`property_media/${temporaryId}/`}
